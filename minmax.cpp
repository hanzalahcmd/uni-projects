...
public:
	...
	void makechildren(int de, int alpha = INT_MIN, int beta = INT_MAX)
	{
		if (de >= 0)
		{
			int min_ = INT_MAX; // Initial high value for minimization
			...
			for (int i = 0; i < 4; ++i)
			{
				for (int j = 0; j < 4; ++j)
				{
					if (state[i][j] == 0)
					{
						...
						// If we find a better move, update beta and favchild
						if (min_ < beta) {
							beta = min_;
							favchild = std::move(child); // Move the child to favchild
						}
	...
	void display()
	{
		// Assuming dis() is a function for displaying the state
		dis(state);
		cout << "\n"
...
void winmove::makechildren(int de, int alpha, int beta)
{
	...
	if (de >= 0)
	{
		...
		if (upmove(temp, a))
		{
			...
			if (max_ > alpha) {
				alpha = max_;
				favchild = std::move(child); // Move only once to favchild
			}
			if (alpha >= beta) // Alpha-beta pruning
				return;
			children.push_back(std::move(favchild)); // Move favchild to children
		}
